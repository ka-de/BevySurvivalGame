...
pub fn handle_spawn_inv_item_tooltip(
    ...
    for item in updates.iter() {
        ...
        let tooltip = commands
            .spawn((
                SpriteBundle {
                    ...
                    transform: Transform {
                        translation: Vec3::new(-(parent_inv_size.x + size.x + 2.0) / 2.0, 0.0, 4.0),
        ...
        tooltip_text.push((item.item_stack.metadata.name.clone(), 0.0));

        if should_show_attributes {
            for a in attributes.iter().clone() {
                tooltip_text.push((a.to_string(), 0.0));
            }
            if let Some(level) = level {
                tooltip_text.push((
                    "Level ".to_string() + &level.to_string(),
                    size.y - ((tooltip_text.len() + 1) as f32) * 10.0 - 14.0,
                ));
            }
        } else {
            for desc_string in item.item_stack.metadata.desc.iter().clone() {
                tooltip_text.push((desc_string.to_string(), 0.0));
            }
        }
        ...
        for (i, (text, d)) in tooltip_text.iter().enumerate() {
            let text_pos = if i == 0 {
                Vec3::new(
                    -f32::ceil(((text.chars().count() * 6 - 1) as f32) / 2.0) + 0.5,
                    size.y / 2.0 - 12.0,
                    1.0
                )
            } else {
                Vec3::new(
                    -size.x / 2.0 + 8.0 + (if item.is_recipe { 4.0 } else { 0.0 }),
                    size.y / 2.0 -
                        14.0 -
                        (i as f32) * (if item.is_recipe { 14.0 } else { 10.0 }) -
                        d,
                    1.0
                )
            };
            ...
            if item.is_recipe && i > 0 {
                let ingredient_world_obj: Vec<WorldObject> = recipes.crafting_list
                ...
                commands
                    ...
                    .insert(Transform {
                        translation: text_pos + Vec3::new(20.0, 0.0, 0.0),
                        ..Default::default()
                    });
        ...
        if let Some(ability) = &item.item_stack.metadata.item_ability {
            ...
            let pos = Vec3::new(28.0, -size.y / 2.0 + 12.0, 1.0);
            let icon_e = spawn_item_stack_icon(
                &mut commands,
                &graphics,
                &(ItemStack {
            ...
            let text = commands
                .spawn((
                    Text2dBundle {
                        text: Text::from_section(format!("{:}", dmg), TextStyle {
                            font: asset_server.load("fonts/Kitchen Sink.ttf"),
                            font_size: 8.0,
                            color: GREY,
                        }),
                        text_anchor: Anchor::CenterLeft,
                        transform: Transform {
                            translation: pos + Vec3::new(-14.0, -3.0, 0.0),
                            scale: Vec3::new(1.0, 1.0, 1.0),
                            ..Default::default()
                        },
                        ..default()
        ...
        if let Ok(inv) = inv.get_single() {
            ...
            commands.entity(essence.get_single().unwrap()).add_child(tooltip);
        }
    }
}
...
pub fn handle_spawn_inv_player_stats(
    ...
    if updates.iter().len() > 0 && tooltip_manager.timer.finished() {
        ...
        let (Ok(parent_e), translation) = if curr_ui_state.0 == UIState::Inventory {
            (
                inv.get_single(),
                Vec3::new(-(INVENTORY_UI_SIZE.x + TOOLTIP_UI_SIZE.x + 2.0) / 2.0, 0.0, 2.0),
            )
        } else if curr_ui_state.0 == UIState::Stats {
            (
                stats.get_single(),
                Vec3::new(-(STATS_UI_SIZE.x + TOOLTIP_UI_SIZE.x + 2.0) / 2.0, 0.0, 2.0),
            )
        } else {
            return;
        } else {
        ...
        tooltip_text.push((("Stats".to_string(), "".to_string()), 0.0));
        for (_i, a) in attributes.iter().enumerate().clone() {
            tooltip_text.push(((a.0.clone(), a.1.clone()), 0.0));
        }

        for (i, (text, d)) in tooltip_text.iter().enumerate() {
            let text_pos = if i == 0 {
                Vec3::new(
                    -f32::ceil(((text.0.chars().count() * 6 - 1) as f32) / 2.0) + 0.5,
                    TOOLTIP_UI_SIZE.y / 2.0 - 12.0,
                    1.0
                )
            } else {
                Vec3::new(
                    -TOOLTIP_UI_SIZE.x / 2.0 + 8.0,
                    TOOLTIP_UI_SIZE.y / 2.0 - 12.0 - (i as f32) * 8.0 - d - 2.0,
                    1.0
                )
            };
            let text = commands
